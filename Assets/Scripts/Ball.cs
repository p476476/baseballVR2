...
public class Ball : MonoBehaviour {
    ...
    private bool struck;        //表示 : 被打到了嗎
    private bool unHitField;   // 表示 : 碰過地板了嗎
    ...
    void OnCollisionEnter(Collider col_event) //col_event為自定義碰撞事件
    {
        if (state == State.Flying2 && col_event.gameObject.name == "Ground") //如果col_event碰撞事件的物件名稱是Ground
        {
    ...
    {
        //成功打擊
        if (col.gameObject.name == "Bat" && !struck)
        {
            if(changeSpeed-1.0f < -0.2f || changeSpeed - 1.0f > 0.2f)
                ScoreManager.GetComponent<CalPlayerData>().Strike(5);

            ScoreManager.GetComponent<CalPlayerData>().Strike((int)type);
            ScoreManager.GetComponent<CalPlayerData>().speedScore(rb.velocity.magnitude);
            ScoreManager.GetComponent<CalPlayerData>().lifeUpdate(true);
            struck = true;
        }
        //沒打到球
        if (col.gameObject.name == "Ground" && !struck && unHitField)
        {
            ScoreManager.GetComponent<CalPlayerData>().lifeUpdate(false);
            unHitField = false;
        }
        //打擊後落地
        if (col.gameObject.name == "Ground" && struck && unHitField)
    ...
    public void setShot(float ball_speed,Type ball_type,float change_speed) {//球速,球種,變速(進壘時速度是投出時的幾倍 1=不變)
